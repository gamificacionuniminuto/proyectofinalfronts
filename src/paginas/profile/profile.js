...
const Profile = () => {
  ...
  const completeActivity = (index) => {
    if (!userData?.student?.isPremium) return;
          ...
          <div className="header-content">
            <div className="avatar-container">
              ...
              <div className="welcome-message">
                ...
                {userData.student.isPremium && <span className="premium-badge">PREMIUM</span>}
              </div>
            </div>
        ...
        <div className="profile-content">
              ...
              <div className="activities-list">
                {userData.activities.map((activity, index) => (
                  <div key={index} className={`activity-card ${activity.completed ? 'completed' : ''}`}>
                    ...
                    {!activity.completed && (
                      <button
                        className={`complete-btn ${!userData.student.isPremium ? 'disabled' : ''}`}
                        onClick={() => completeActivity(index)}
                        disabled={!userData.student.isPremium}
                      >
                        {userData.student.isPremium ? 'Completar' : 'Premium requerido'}
                      </button>
                    )}
                  </div>
                ))}
          ...
          {activeTab === 'logros' && (
            userData.student.isPremium ? (
              ...
              <section className="premium-message">
                <h2>🔒 Contenido Premium</h2>
                <p>Desbloquea todos los logros y características premium con una suscripción.</p>
                <button onClick={() => navigate('/subscribe')}>Suscribirse</button>
              </section>
            )
          )}
          ...
          {activeTab === 'info' && (
            <section className="info-section">
              <div className="info-card kid-info">
                ...
                <p><strong>Estado:</strong> {userData.student.isPremium ? 'Premium 🎁' : 'Básico 🔒'}</p>
              </div>
          ...
          <section className="quick-actions">
            ...
            <div className="actions-grid">
              ...
              <button 
                ...
                onClick={() => userData.student.isPremium 
                  ? navigate('/Chatbot') 
                  : alert('Necesitas suscripción premium para acceder al asistente')}
                style={!userData.student.isPremium ? { opacity: 0.5, filter: 'grayscale(50%)' } : {}}
              >
                <img src={Conejo} alt="Icono de conejo" className="icono-conejo" />
                <div className="burbuja-texto">¡Soy Mati!</div>
